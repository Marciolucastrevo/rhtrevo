-- RHTrevo: recovery migration for a partially applied foundation migration.
-- Safe to run in the Rhtrevo development project after the initial script stopped
-- after creating enum types. It creates missing tables and replaces the auth trigger.
...
create unique index if not exists employee_assignments_one_primary_active
...
create unique index if not exists employee_relationships_one_active_direct_manager
...
create table if not exists public.assignment_responsibilities (
  id uuid primary key default gen_random_uuid(),
...
create table if not exists public.autonomy_levels (
  id uuid primary key default gen_random_uuid(),
  name text not null unique,
  description text,
  active boolean not null default true,
...
create table if not exists public.autonomy_level_permissions (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists audit_logs_entity_lookup on public.audit_logs(entity_type, entity_id, created_at desc);
create index if not exists employee_assignments_employee_lookup on public.employee_assignments(employee_id, starts_at desc);
create index if not exists employee_relationships_subject_lookup on public.employee_relationships(subject_employee_id, kind, starts_at desc);
create index if not exists user_permission_grants_user_lookup on public.user_permission_grants(user_id, permission_key, starts_at desc);
