...
type ModuleKey = 'home' | 'structure' | 'people' | 'journey' | 'responsibilities' | 'organogram' | 'imports' | 'authorizations' | 'audit'
...
type EmployeeDocumentType = { id: string; name: string; category: string; default_classification: string; requires_expiry: boolean; active: boolean; description: string | null }
type EmployeeDocument = { id: string; employee_id: string; document_type_id: string | null; title: string; reference_code: string | null; classification: string; status: string; issued_at: string | null; expires_at: string | null; reviewed_at: string | null; document_url: string | null; notes: string | null; is_confidential: boolean; created_at: string }
type EmployeeOccurrence = { id: string; employee_id: string; occurrence_type: string; severity: string; occurred_on: string; description: string; is_confidential: boolean; status: string; resolved_at: string | null; created_at: string }
type EmployeeCommunication = { id: string; employee_id: string; subject: string; message: string; channel: string; sent_at: string | null; read_at: string | null; is_confidential: boolean; status: string; created_at: string }
...
function App() {
  ...
  const [documentTypes, setDocumentTypes] = useState<EmployeeDocumentType[]>([])
  const [employeeDocuments, setEmployeeDocuments] = useState<EmployeeDocument[]>([])
  const [employeeOccurrences, setEmployeeOccurrences] = useState<EmployeeOccurrence[]>([])
  const [employeeCommunications, setEmployeeCommunications] = useState<EmployeeCommunication[]>([])
  ...
  async function refreshStructure() {
    ...
    const [companyResult, areaResult, unitResult, jobRoleResult, teamResult, employeeResult, assignmentResult, relationshipResult, responsibilityCatalogResult, assignmentResponsibilityResult, profileResult, permissionCatalogResult, permissionGrantResult, auditLogResult, documentTypeResult, documentResult, occurrenceResult, communicationResult] = await Promise.all([
      ...
      supabase.from('employee_document_types').select('id, name, category, default_classification, requires_expiry, active, description').order('category').order('name'),
      supabase.from('employee_documents').select('id, employee_id, document_type_id, title, reference_code, classification, status, issued_at, expires_at, reviewed_at, document_url, notes, is_confidential, created_at').order('created_at', { ascending: false }),
      supabase.from('employee_occurrences').select('id, employee_id, occurrence_type, severity, occurred_on, description, is_confidential, status, resolved_at, created_at').order('occurred_on', { ascending: false }),
      supabase.from('employee_communications').select('id, employee_id, subject, message, channel, sent_at, read_at, is_confidential, status, created_at').order('created_at', { ascending: false }),
    ])
    const error = companyResult.error ?? areaResult.error ?? unitResult.error ?? jobRoleResult.error ?? teamResult.error ?? employeeResult.error ?? assignmentResult.error ?? relationshipResult.error ?? responsibilityCatalogResult.error ?? assignmentResponsibilityResult.error ?? profileResult.error ?? permissionCatalogResult.error ?? permissionGrantResult.error ?? auditLogResult.error ?? documentTypeResult.error ?? documentResult.error ?? occurrenceResult.error ?? communicationResult.error
    ...
    setDocumentTypes((documentTypeResult.data ?? []) as EmployeeDocumentType[])
    setEmployeeDocuments((documentResult.data ?? []) as EmployeeDocument[])
    setEmployeeOccurrences((occurrenceResult.data ?? []) as EmployeeOccurrence[])
    setEmployeeCommunications((communicationResult.data ?? []) as EmployeeCommunication[])
  ...
  return <main className="app-shell">
    ...
    {isRootAdmin && <nav className="module-nav" aria-label="Módulos da plataforma">{([{ key: 'home', label: 'Visão geral' }, { key: 'structure', label: 'Estrutura' }, { key: 'people', label: 'Pessoas & vínculos' }, { key: 'journey', label: 'Jornada' }, { key: 'responsibilities', label: 'Responsabilidades' }, { key: 'organogram', label: 'Organograma' }, { key: 'imports', label: 'Importações' }, { key: 'authorizations', label: 'Autorizações' }, { key: 'audit', label: 'Auditoria' }] as Array<{ key: ModuleKey; label: string }>).map((module) => <button className={activeModule === module.key ? 'module-active' : ''} type="button" key={module.key} onClick={() => setActiveModule(module.key)}>{module.label}</button>)}</nav>}
    ...
    {isRootAdmin && activeModule === 'journey' && <JourneyModule employees={employees} selectedEmployeeId={selectedEmployeeId} onSelectEmployee={setSelectedEmployeeId} documentTypes={documentTypes} documents={employeeDocuments} occurrences={employeeOccurrences} communications={employeeCommunications} onRefresh={() => void refreshStructure()} loading={structureLoading} onError={setStructureError} onNotice={setOperationNotice} />}
...
function JourneyModule({ employees, selectedEmployeeId, onSelectEmployee, documentTypes, documents, occurrences, communications, onRefresh, loading, onError, onNotice }: { employees: Employee[]; selectedEmployeeId: string; onSelectEmployee: (id: string) => void; documentTypes: EmployeeDocumentType[]; documents: EmployeeDocument[]; occurrences: EmployeeOccurrence[]; communications: EmployeeCommunication[]; onRefresh: () => void; loading: boolean; onError: (message: string | null) => void; onNotice: (message: string | null) => void }) {
  const [query, setQuery] = useState('')
  const [documentStatus, setDocumentStatus] = useState('')
  const [occurrenceStatus, setOccurrenceStatus] = useState('')
  const [communicationStatus, setCommunicationStatus] = useState('')
  const selectedEmployee = employees.find((employee) => employee.id === selectedEmployeeId)
  const personDocuments = documents.filter((item) => item.employee_id === selectedEmployeeId && (!documentStatus || item.status === documentStatus))
  const personOccurrences = occurrences.filter((item) => item.employee_id === selectedEmployeeId && (!occurrenceStatus || item.status === occurrenceStatus))
  const personCommunications = communications.filter((item) => item.employee_id === selectedEmployeeId && (!communicationStatus || item.status === communicationStatus))
  const visibleEmployees = employees.filter((employee) => employee.full_name.toLocaleLowerCase().includes(query.toLocaleLowerCase()))
  const today = new Date().toISOString().slice(0, 10)
  const dateTimeLocalNow = new Date().toISOString().slice(0, 16)
  const submit = async (event: FormEvent<HTMLFormElement>, table: 'employee_document_types' | 'employee_documents' | 'employee_occurrences' | 'employee_communications', data: Record<string, unknown>, success: string) => {
    event.preventDefault()
    if (!supabase) return
    onError(null)
    const { error } = await supabase.from(table).insert(data)
    if (error) return onError(`Não foi possível registrar o item da jornada: ${error.message}`)
    event.currentTarget.reset()
    onNotice(success)
    onRefresh()
  }
  const formatDate = (value: string | null) => value ? new Intl.DateTimeFormat('pt-BR', { dateStyle: 'medium' }).format(new Date(`${value.slice(0, 10)}T12:00:00`)) : '—'
  const typeName = (id: string | null) => documentTypes.find((type) => type.id === id)?.name ?? 'Sem tipo definido'
  const statusClass = (status: string) => `journey-status journey-${status.replaceAll('_', '-')}`
  return <section className="journey-section"><div className="section-heading"><div><p className="eyebrow">Jornada do colaborador</p><h2>Documentos, ocorrências e comunicados</h2><p>Registros operacionais por pessoa, com referência externa opcional e sem armazenamento de arquivos.</p></div><button className="outline-button" type="button" onClick={onRefresh} disabled={loading}>{loading ? 'Atualizando…' : 'Atualizar'}</button></div><div className="journey-layout"><aside className="journey-sidebar"><label>Selecionar pessoa<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Nome do colaborador" /></label><div className="journey-person-list">{visibleEmployees.map((employee) => <button className={employee.id === selectedEmployeeId ? 'person-selected' : ''} type="button" key={employee.id} onClick={() => onSelectEmployee(employee.id)}>{employee.full_name}</button>)}</div><article className="journey-type-card"><h3>Tipos de documento</h3><p>Padronize categorias e validade esperada.</p><form className="journey-form" onSubmit={(event) => { const form = new FormData(event.currentTarget); void submit(event, 'employee_document_types', { name: String(form.get('name')).trim(), category: String(form.get('category')), default_classification: String(form.get('default_classification')), requires_expiry: form.get('requires_expiry') === 'on', description: String(form.get('description')).trim() || null }, 'Tipo de documento adicionado.') }}><input name="name" placeholder="Ex.: ASO periódico" required /><select name="category" defaultValue="employment"><option value="identity">Identificação</option><option value="employment">Trabalhista</option><option value="benefits">Benefícios</option><option value="health_safety">Saúde e segurança</option><option value="training">Treinamento</option><option value="performance">Desempenho</option><option value="other">Outro</option></select><select name="default_classification" defaultValue="document"><option value="document">Documento</option><option value="personal">Pessoal</option><option value="medical">Médica</option><option value="performance">Desempenho</option></select><label className="checkbox-label"><input name="requires_expiry" type="checkbox" /> Exige validade</label><input name="description" placeholder="Descrição (opcional)" /><button type="submit">Adicionar tipo</button></form><div className="journey-type-list">{documentTypes.map((type) => <div key={type.id}><strong>{type.name}</strong><small>{type.category} · {type.requires_expiry ? 'com validade' : 'sem validade'}</small></div>)}</div></article></aside><div className="journey-main">{selectedEmployee ? <><div className="person-title"><div><p className="eyebrow">Pessoa selecionada</p><h3>{selectedEmployee.full_name}</h3></div><span className={selectedEmployee.active ? 'record-active' : 'record-inactive'}>{selectedEmployee.active ? 'Ativo' : 'Inativo'}</span></div><div className="journey-grid"><article className="journey-card"><h3>Novo documento</h3><form className="journey-form" onSubmit={(event) => { const form = new FormData(event.currentTarget); void submit(event, 'employee_documents', { employee_id: selectedEmployeeId, document_type_id: String(form.get('document_type_id')) || null, title: String(form.get('title')).trim(), reference_code: String(form.get('reference_code')).trim() || null, classification: String(form.get('classification')), status: String(form.get('status')), issued_at: String(form.get('issued_at')) || null, expires_at: String(form.get('expires_at')) || null, reviewed_at: String(form.get('reviewed_at')) || null, document_url: String(form.get('document_url')).trim() || null, notes: String(form.get('notes')).trim() || null, is_confidential: form.get('is_confidential') === 'on' }, 'Documento registrado na jornada.') }}><select name="document_type_id" defaultValue=""><option value="">Tipo (opcional)</option>{documentTypes.filter((type) => type.active).map((type) => <option value={type.id} key={type.id}>{type.name}</option>)}</select><input name="title" placeholder="Título ou identificação" required /><input name="reference_code" placeholder="Referência (opcional)" /><select name="classification" defaultValue="document"><option value="document">Documento</option><option value="personal">Pessoal</option><option value="medical">Médica</option><option value="performance">Desempenho</option></select><select name="status" defaultValue="pending"><option value="pending">Pendente</option><option value="valid">Válido</option><option value="under_review">Em revisão</option><option value="rejected">Rejeitado</option><option value="archived">Arquivado</option></select><label>Emissão<input name="issued_at" type="date" /></label><label>Validade<input name="expires_at" type="date" /></label><label>Revisão<input name="reviewed_at" type="date" /></label><input name="document_url" type="url" placeholder="URL externa opcional (https://)" /><textarea name="notes" placeholder="Metadados e observações (opcional)" /><label className="checkbox-label"><input name="is_confidential" type="checkbox" /> Confidencial</label><button type="submit">Registrar documento</button></form></article><article className="journey-card"><h3>Nova ocorrência</h3><form className="journey-form" onSubmit={(event) => { const form = new FormData(event.currentTarget); void submit(event, 'employee_occurrences', { employee_id: selectedEmployeeId, occurrence_type: String(form.get('occurrence_type')).trim(), severity: String(form.get('severity')), occurred_on: String(form.get('occurred_on')), description: String(form.get('description')).trim(), is_confidential: form.get('is_confidential') === 'on', status: String(form.get('status')) }, 'Ocorrência registrada na jornada.') }}><input name="occurrence_type" placeholder="Tipo da ocorrência" required /><select name="severity" defaultValue="low"><option value="low">Baixa</option><option value="medium">Média</option><option value="high">Alta</option><option value="critical">Crítica</option></select><label>Data<input name="occurred_on" type="date" defaultValue={today} required /></label><select name="status" defaultValue="open"><option value="open">Aberta</option><option value="under_review">Em análise</option><option value="resolved">Resolvida</option><option value="dismissed">Encerrada sem ação</option></select><textarea name="description" placeholder="Descrição da ocorrência" required /><label className="checkbox-label"><input name="is_confidential" type="checkbox" /> Confidencial</label><button type="submit">Registrar ocorrência</button></form></article><article className="journey-card"><h3>Novo comunicado</h3><form className="journey-form" onSubmit={(event) => { const form = new FormData(event.currentTarget); const sentAt = String(form.get('sent_at')); const readAt = String(form.get('read_at')); void submit(event, 'employee_communications', { employee_id: selectedEmployeeId, subject: String(form.get('subject')).trim(), message: String(form.get('message')).trim(), channel: String(form.get('channel')), sent_at: sentAt ? new Date(sentAt).toISOString() : null, read_at: readAt ? new Date(readAt).toISOString() : null, is_confidential: form.get('is_confidential') === 'on', status: String(form.get('status')) }, 'Comunicado registrado na jornada.') }}><input name="subject" placeholder="Assunto" required /><select name="channel" defaultValue="platform"><option value="platform">Plataforma</option><option value="email">E-mail</option><option value="whatsapp">WhatsApp</option><option value="in_person">Presencial</option><option value="other">Outro</option></select><select name="status" defaultValue="draft"><option value="draft">Rascunho</option><option value="sent">Enviado</option><option value="read">Lido</option><option value="archived">Arquivado</option></select><label>Enviado em<input name="sent_at" type="datetime-local" defaultValue={dateTimeLocalNow} /></label><label>Lido em<input name="read_at" type="datetime-local" /></label><textarea name="message" placeholder="Mensagem do comunicado" required /><label className="checkbox-label"><input name="is_confidential" type="checkbox" /> Confidencial</label><button type="submit">Registrar comunicado</button></form></article></div><div className="journey-lists"><article className="journey-card"><div className="journey-list-heading"><h3>Documentos</h3><select value={documentStatus} onChange={(event) => setDocumentStatus(event.target.value)}><option value="">Todos os status</option><option value="pending">Pendente</option><option value="valid">Válido</option><option value="expired">Expirado</option><option value="under_review">Em revisão</option><option value="rejected">Rejeitado</option><option value="archived">Arquivado</option></select></div>{personDocuments.length === 0 ? <p className="empty-state">Nenhum documento para este filtro.</p> : <div className="journey-record-list">{personDocuments.map((item) => <div key={item.id}><div><strong>{item.title}</strong><small>{typeName(item.document_type_id)} · {item.classification} · validade {formatDate(item.expires_at)}</small>{item.document_url && <a href={item.document_url} target="_blank" rel="noreferrer">Abrir referência externa</a>}</div><div><span className={statusClass(item.status)}>{item.status}</span>{item.is_confidential && <span className="journey-confidential">Confidencial</span>}</div></div>)}</div>}</article><article className="journey-card"><div className="journey-list-heading"><h3>Ocorrências</h3><select value={occurrenceStatus} onChange={(event) => setOccurrenceStatus(event.target.value)}><option value="">Todos os status</option><option value="open">Aberta</option><option value="under_review">Em análise</option><option value="resolved">Resolvida</option><option value="dismissed">Encerrada</option></select></div>{personOccurrences.length === 0 ? <p className="empty-state">Nenhuma ocorrência para este filtro.</p> : <div className="journey-record-list">{personOccurrences.map((item) => <div key={item.id}><div><strong>{item.occurrence_type}</strong><small>{formatDate(item.occurred_on)} · severidade {item.severity}</small><p>{item.description}</p></div><div><span className={statusClass(item.status)}>{item.status}</span>{item.is_confidential && <span className="journey-confidential">Confidencial</span>}</div></div>)}</div>}</article><article className="journey-card"><div className="journey-list-heading"><h3>Comunicados</h3><select value={communicationStatus} onChange={(event) => setCommunicationStatus(event.target.value)}><option value="">Todos os status</option><option value="draft">Rascunho</option><option value="sent">Enviado</option><option value="read">Lido</option><option value="archived">Arquivado</option></select></div>{personCommunications.length === 0 ? <p className="empty-state">Nenhum comunicado para este filtro.</p> : <div className="journey-record-list">{personCommunications.map((item) => <div key={item.id}><div><strong>{item.subject}</strong><small>{item.channel} · enviado {item.sent_at ? new Intl.DateTimeFormat('pt-BR', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(item.sent_at)) : 'não informado'}</small><p>{item.message}</p></div><div><span className={statusClass(item.status)}>{item.status}</span>{item.is_confidential && <span className="journey-confidential">Confidencial</span>}</div></div>)}</div>}</article></div></> : <p className="empty-state">Selecione uma pessoa para administrar a jornada.</p>}</div></div></section>
}
