...
function Organogram({ companies, units, areas, teams, employees, assignments, relationships, jobRoles }: { companies: Company[]; units: Unit[]; areas: Area[]; teams: Team[]; employees: Employee[]; assignments: Assignment[]; relationships: Relationship[]; jobRoles: JobRole[] }) {
  ...
  const scopedRelations = relationships.filter((relationship) => !relationship.ends_at && scopedPeople.some((person) => person.id === relationship.subject_employee_id || person.id === relationship.related_employee_id))
