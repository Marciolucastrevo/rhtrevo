...
function Organogram({ companies, units, areas, employees, assignments, relationships }: { companies: Company[]; units: Unit[]; areas: Area[]; employees: Employee[]; assignments: Assignment[]; relationships: Relationship[] }) {
  ...
  return <section className="organogram-section"><div className="section-heading"><div><p className="eyebrow">Organograma operacional</p><h2>Fluxograma organizacional</h2><p>Selecione o escopo para visualizar os vínculos e ligações existentes.</p></div></div><div className="organogram-filters"><select value={scope} onChange={(event) => { setScope(event.target.value as typeof scope); setScopeId('') }}><option value="company">Por empresa</option><option value="area">Por área</option><option value="unit">Por unidade</option><option value="person">Por pessoa</option></select><select value={scopeId} onChange={(event) => setScopeId(event.target.value)}><option value="">Selecione o filtro</option>{options.filter((item) => item.active).map((item) => <option value={item.id} key={item.id}>{item.name ?? item.full_name}</option>)}</select></div>{scopeId && <div className="flowchart"><div className="flow-root">{options.find((item) => item.id === scopeId)?.name ?? options.find((item) => item.id === scopeId)?.full_name}</div><div className="flow-branches">{scopedPeople.map((person) => <div className="flow-node" key={person.id}><strong>{person.full_name}</strong><small>{scoped.find((assignment) => assignment.employee_id === person.id)?.kind ?? 'vínculo'}</small>{scopedRelations.filter((relationship) => relationship.subject_employee_id === person.id).map((relationship) => <span key={relationship.id}>→ {employees.find((employee) => employee.id === relationship.related_employee_id)?.full_name} ({relationship.kind})</span>)}</div>)}</div></div>}<p className="organogram-note">{scopedPeople.length} pessoas · {scoped.length} vínculos · {scopedRelations.length} ligações no escopo</p></section>
}
