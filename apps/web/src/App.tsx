...
function App() {
  ...
  return <main className="app-shell">
    ...
    {isRootAdmin && activeModule === 'organogram' && <Organogram companies={companies} units={units} areas={areas} employees={employees} assignments={assignments} relationships={relationships} jobRoles={jobRoles} />}
...
function Organogram({ companies, units, areas, employees, assignments, relationships, jobRoles }: { companies: Company[]; units: Unit[]; areas: Area[]; employees: Employee[]; assignments: Assignment[]; relationships: Relationship[]; jobRoles: JobRole[] }) {
  const [scope, setScope] = useState<'company' | 'area' | 'unit' | 'person'>('company')
  const [scopeId, setScopeId] = useState('')
  const options = scope === 'company' ? companies : scope === 'area' ? areas : scope === 'unit' ? units : employees
  ...
  const scoped = scope === 'company' ? activeAssignments.filter((assignment) => assignment.company_id === scopeId) : scope === 'area' ? activeAssignments.filter((assignment) => assignment.area_id === scopeId) : scope === 'unit' ? activeAssignments.filter((assignment) => assignment.unit_id === scopeId) : activeAssignments.filter((assignment) => assignment.employee_id === scopeId)
  const scopedPeople = employees.filter((employee) => scoped.some((assignment) => assignment.employee_id === employee.id))
  const scopedRelations = relationships.filter((relationship) => !relationship.ends_at && (scopedPeople.some((person) => person.id === relationship.subject_employee_id) || scopedPeople.some((person) => person.id === relationship.related_employee_id)))
  const selectedPerson = employees.find((employee) => employee.id === scopeId)
  const filters = <div className="organogram-filters"><select value={scope} onChange={(event) => { setScope(event.target.value as typeof scope); setScopeId('') }}><option value="company">Por empresa</option><option value="area">Por área</option><option value="unit">Por unidade</option><option value="person">Por pessoa</option></select><select value={scopeId} onChange={(event) => setScopeId(event.target.value)}><option value="">Selecione o filtro</option>{options.filter((item) => item.active).map((item) => <option value={item.id} key={item.id}>{labelOf(item)}</option>)}</select></div>
  if (scope === 'person' && selectedPerson) return <section className="organogram-section"><div className="section-heading"><div><p className="eyebrow">Organograma operacional</p><h2>Pessoa 360º</h2><p>Relações, vínculos e responsabilidades do perfil selecionado.</p></div></div>{filters}<Person360 person={selectedPerson} assignments={scoped} relationships={relationships.filter((relationship) => !relationship.ends_at)} employees={employees} companies={companies} units={units} areas={areas} jobRoles={jobRoles} /></section>
  return <section className="organogram-section"><div className="section-heading"><div><p className="eyebrow">Organograma operacional</p><h2>Fluxograma organizacional</h2><p>Selecione o escopo para visualizar os vínculos e ligações existentes.</p></div></div>{filters}{scopeId && <div className="flowchart"><div className="flow-root">{labelOf(options.find((item) => item.id === scopeId) ?? {})}</div><div className="flow-branches">{scopedPeople.map((person) => <div className="flow-node" key={person.id}><strong>{person.full_name}</strong><small>{scoped.find((assignment) => assignment.employee_id === person.id)?.kind ?? 'vínculo'}</small>{scopedRelations.filter((relationship) => relationship.subject_employee_id === person.id).map((relationship) => <span key={relationship.id}>→ {employees.find((employee) => employee.id === relationship.related_employee_id)?.full_name} ({relationship.kind})</span>)}</div>)}</div></div>}<p className="organogram-note">{scopedPeople.length} pessoas · {scoped.length} vínculos · {scopedRelations.length} ligações no escopo</p></section>
}

function Person360({ person, assignments, relationships, employees, companies, units, areas, jobRoles }: { person: Employee; assignments: Assignment[]; relationships: Relationship[]; employees: Employee[]; companies: Company[]; units: Unit[]; areas: Area[]; jobRoles: JobRole[] }) {
  ...
  const managerRelations = relationships.filter((relationship) => relationship.subject_employee_id === person.id && ['direct_manager', 'secondary_manager'].includes(relationship.kind))
  const responsibilityRelations = relationships.filter((relationship) => relationship.subject_employee_id === person.id && !['direct_manager', 'secondary_manager'].includes(relationship.kind))
  const subordinateRelations = relationships.filter((relationship) => relationship.related_employee_id === person.id)
  ...
  const managers = managerRelations.length ? <div className="person360-list">{managerRelations.map((relationship) => <div key={relationship.id}><strong>{personName(relationship.related_employee_id)}</strong><small>{labels[relationship.kind] ?? relationship.kind}</small></div>)}</div> : empty
  ...
  const responsibilities = responsibilityRelations.length ? <div className="person360-list">{responsibilityRelations.map((relationship) => <div key={relationship.id}><strong>{personName(relationship.related_employee_id)}</strong><small>{labels[relationship.kind] ?? relationship.kind}</small></div>)}</div> : empty
  const subordinates = subordinateRelations.length ? <div className="person360-list">{subordinateRelations.map((relationship) => <div key={relationship.id}><strong>{personName(relationship.subject_employee_id)}</strong><small>{labels[relationship.kind] ?? relationship.kind}</small></div>)}</div> : empty
