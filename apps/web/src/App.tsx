...
type ModuleKey = 'home' | 'structure' | 'people' | 'responsibilities' | 'organogram' | 'authorizations' | 'audit'
...
function App() {
  ...
  return <main className="app-shell">
    ...
    {isRootAdmin && <nav className="module-nav" aria-label="Módulos da plataforma">{([{ key: 'home', label: 'Visão geral' }, { key: 'structure', label: 'Estrutura' }, { key: 'people', label: 'Pessoas & vínculos' }, { key: 'responsibilities', label: 'Responsabilidades' }, { key: 'organogram', label: 'Organograma' }, { key: 'authorizations', label: 'Autorizações' }, { key: 'audit', label: 'Auditoria' }] as Array<{ key: ModuleKey; label: string }>).map((module) => <button className={activeModule === module.key ? 'module-active' : ''} type="button" key={module.key} onClick={() => setActiveModule(module.key)}>{module.label}</button>)}</nav>}
