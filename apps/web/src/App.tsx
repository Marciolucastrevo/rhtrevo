...
function Person360({ person, assignments, relationships, employees, companies, jobRoles }: { person: Employee; assignments: Assignment[]; relationships: Relationship[]; employees: Employee[]; companies: Company[]; jobRoles: JobRole[] }) {
  const managers = relationships.filter((relationship) => relationship.subject_employee_id === person.id)
  const subordinates = relationships.filter((relationship) => relationship.related_employee_id === person.id)
  const cards = (title: string, className: string, content: any) => <article className={`person360-card ${className}`}><h3>{title}</h3>{content}</article>
  const list = (items: any[]) => items.length ? <div className="person360-list">{items}</div> : <p className="empty-state">Nada por aqui ainda.</p>
  return <div className="person360">{cards('Empresas', 'companies', list(assignments.map((assignment) => <div key={assignment.id}><strong>{companies.find((company) => company.id === assignment.company_id)?.name ?? 'Sem empresa'}</strong><small>{assignment.kind}</small></div>)))}{cards('Gestores', 'managers', list(managers.map((relationship) => <div key={relationship.id}><strong>{employees.find((employee) => employee.id === relationship.related_employee_id)?.full_name}</strong><small>{relationship.kind}</small></div>)))}{cards('Cargo', 'roles', list(assignments.map((assignment) => <div key={assignment.id}><strong>{jobRoles.find((role) => role.id === assignment.job_role_id)?.name ?? 'Sem cargo'}</strong></div>)))}{cards('Papéis', 'responsibilities', <p className="empty-state">Nada por aqui ainda.</p>)}<article className="person360-center"><div className="person360-avatar">{person.full_name.split(' ').slice(0,2).map((part) => part[0]).join('')}</div><h3>{person.full_name}</h3><p>{jobRoles.find((role) => role.id === assignments[0]?.job_role_id)?.name ?? 'Sem cargo definido'}</p><small>{companies.find((company) => company.id === assignments[0]?.company_id)?.name ?? 'Sem empresa definida'}</small></article>{cards('Avaliadores', 'evaluators', <p className="empty-state">Nada por aqui ainda.</p>)}{cards('Subordinados', 'subordinates', list(subordinates.map((relationship) => <div key={relationship.id}><strong>{employees.find((employee) => employee.id === relationship.subject_employee_id)?.full_name}</strong><small>{relationship.kind}</small></div>)))}{cards('Contratos', 'contracts', list(assignments.filter((assignment) => assignment.kind === 'employment').map((assignment) => <div key={assignment.id}><strong>Vínculo empregatício</strong><small>{assignment.starts_at} · vigente</small></div>)))}</div>
}
