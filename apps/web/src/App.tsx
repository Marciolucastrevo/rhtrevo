...
function App() {
  ...
  function validateImportRows(rawRows: string[][]): ImportRelationship[] {
    ...
    const expected = [
      ['operacao'], ['tipo_relacao', 'tiporelacao'], ['empresa_codigo', 'codigo_empresa', 'company_external_code', 'external_code'],
      ['codigo_colaborador', 'matricula_colaborador', 'subject_employee_code', 'codigo_subordinado'],
      ['codigo_responsavel', 'matricula_responsavel', 'related_employee_code', 'codigo_gestor', 'codigo_gestor_responsavel'],
      ['data_inicio', 'inicio', 'starts_at'],
    ]
    if (expected.some((aliases) => !headers.some((header) => aliases.includes(header)))) return [{ line: 1, operation: '', relationType: '', companyCode: '', subjectCode: '', relatedCode: '', startsAt: '', status: 'blocked' as const, message: 'Cabeçalho inválido. Use: operacao;tipo_relacao;empresa_codigo;codigo_colaborador;codigo_responsavel;data_inicio.' }]
    ...
    return dataRows.map((row, index) => {
      ...
      const companyCode = valueAt(row, ['empresa_codigo', 'codigo_empresa', 'company_external_code', 'external_code']).trim()
      const subjectCode = valueAt(row, ['codigo_colaborador', 'matricula_colaborador', 'subject_employee_code', 'codigo_subordinado']).trim()
      const relatedCode = valueAt(row, ['codigo_responsavel', 'matricula_responsavel', 'related_employee_code', 'codigo_gestor', 'codigo_gestor_responsavel']).trim()
